...
void main() async {
  ...
  final account = await wallet.addAccountFromSecretKey(
      Env.privateKey,
      AddressType.user,
      isBuildnet ? NetworkType.BUILDNET : NetworkType.MAINNET);
...
// covert USDC to exact 1.0 MAS
  final (route, pair, binSteps, amounts, amountsWithoutSlippage, fees) =
      await quoter.findBestPathFromAmountOut(
          TokenName.USDC, TokenName.WMAS, BigInt.from(amountOut));
  ...
  final amountBigInt =
      decimalToBigInt(amountInWithSlippage, getTokenDecimal(TokenName.USDC));
  ...
  final result = await swap.swapTokensForExactMAS(amountBigInt, amounts[1],
      binSteps, route, account.address(), CommonConstants.txDeadline);
  ...
  final (route2, pair2, binSteps2, amounts2, amountsWithoutSlippage2, fees2) =
      await quoter.findBestPathFromAmountOut(
          TokenName.WETH, TokenName.WMAS, BigInt.from(amountOut));
  print('amount out: $amountOut');
  print('route: $route2');
  print('pair: $pair2');
  print('bin steps: $binSteps2');
  final amountIn2 =
      bigIntToDecimal(amounts2[0], getTokenDecimal(TokenName.WETH));
  final amountInWithSlippage2 = maximumAmoutIn(amountIn2, 0.5);
  final amountBigInt2 =
      decimalToBigInt(amountInWithSlippage2, getTokenDecimal(TokenName.WETH));
  ...
  final result2 = await swap.swapTokensForExactMAS(amountBigInt2, amounts2[1],
      binSteps2, route2, account.address(), CommonConstants.txDeadline);
